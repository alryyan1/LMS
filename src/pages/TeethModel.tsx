...
interface TeethModelProbs {
  ...
  user: any;
  setActiveDoctorVisit: (visit: any) => void;
  settings: any;
}
function TeethModel({
  actviePatient,
  ...
  console.log(
    actviePatient.services
      .filter((rs) => {
        return actviePatient.requested_tooth_services
          .filter((rt) => rt.tooth_id == 15)
          .map((rt) => rt.requested_service_id)
          .includes(rs.id);
      })
      .map((rs) => {
  ...
  const toothes = actviePatient.requested_tooth_services.map(
    (ts) => ts.tooth_id
  );
  console.log(toothes, "toothes");
...
c-0.6,1.5-0.3,3.6,1.1,4.2c2.7,1.1,4.3,4.2,3.8,7.1c-1,1-2.2-0.9-2.6-2.2C54.3,272.8,50.8,270.2,47.9,271.6z"
    ...
    {!dayjs(actviePatient.created_at).startOf("day").isBefore(dayjs().startOf("day")) &&  <EmptyDialog title="Service Request" setShow={setShow} show={show}>
        <ServiceGroup
          ...
          setActivePatient={setActiveDoctorVisit}
        />
        <RequestedServices
          actviePatient={actviePatient}
          activeShift={null}
