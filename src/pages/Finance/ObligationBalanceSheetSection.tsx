...
function ObligationBalanceSheetSection({balanceSheet,setBalanceSheet}) {
  ...
  // Inside ObligationBalanceSheetSection.js
  useEffect(() => {
  ...
  const updateTable = async () => {
    try {
      const response = await axiosClient.patch(`updateBalanceSheetStatement/${balanceSheet?.id}`, {
        ...
        obligations: rows, // Send the table data
      });
  ...
  return (
        ...
        <Paper sx={{ p: 2 }}>
          ...
          <Typography variant="h5" textAlign={"center"} gutterBottom>
            ( {balanceSheet?.id} )  حقوق الملكيه + الالتزامات 
          </Typography>
          <TableContainer>
...
export default ObligationBalanceSheetSection;
