...
import { Alert, Autocomplete, Button, Chip, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, Grid, IconButton, InputLabel, MenuItem, Paper, Select, Snackbar, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Tooltip, Typography } from "@mui/material";
...
interface Account { // Define a basic Account type if not imported
  id: number;
  name: string;
}

interface Entry { // Define a basic Entry type if not imported
    id: number;
    hasPetty: boolean;
...
function PettyCashPermissionsTable() {
  ...
  const [managerApprovedFilter, setManagerApprovedFilter] = useState<string>('all'); // 'all', 'approved', 'not_approved'
  const [auditorApprovedFilter, setAuditorApprovedFilter] = useState<string>('all'); // 'all', 'approved', 'not_approved'

  // --- UI / Dialog States ---
  const [deleteConfirmationOpen, setDeleteConfirmationOpen] = useState(false);
  const [permissionToDelete, setPermissionToDelete] = useState<number | null>(null);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [permissionToEdit, setPermissionToEdit] = useState<PettyCashPermission | null>(null);
  ...
  const { control, handleSubmit, reset, setValue, register } = useForm<PettyCashPermission>();
  ...
  const fetchPermissionsData = async (
    ...
    try {
      ...
      if (response.data.data && response.data.links) { // Paginated response
         setAllPermissions(response.data.data);
         setLinks(response.data.links);
      } else if (Array.isArray(response.data)) { // Filter response (might not be paginated)
 ...
 const fetchInitialPermissions = () => {
  ...
  const fetchEntries = async () => {
     try {
       ...
       const response = await axiosClient.get('financeEntries');
        // Assuming response.data is the array or response.data.data
       setEntires(Array.isArray(response.data) ? response.data : response.data?.data || []);
     } catch (error) {
  ...
  const applyFilters = () => {
     ...
     fetchPermissionsData('petty-cash-permissions-filter', {
        ...
        first: firstDate.format("YYYY-MM-DD"), // Consistent format
        second: secondDate.format("YYYY-MM-DD"),
  ...
  const handleEdit = (id: number) => {
   ...
   const handleUpdateEntryLink = (permissionId: number, newEntryId: number | null) => {
       if (!newEntryId) return; // Do nothing if no entry selected

       axiosClient.patch(`setFinanceEntry/${permissionId}`, {
  ...
  const handleApprove = (permissionId: number, colName: 'user_approved_time' | 'auditor_approved_time') => {
      const actionType = colName === 'user_approved_time' ? 'المدير' : 'المراجع';
      let result = confirm(`سيتم اعتماد اذن الصرف من قبل ${actionType}. هل انت متأكد؟`);
      if (result) {
          setLoading(true); // Indicate processing
          axiosClient.get(`expense-approve/${permissionId}?colName=${colName}`)
              .then(({ data }) => {
                  if (data?.data) {
                     // Update the specific permission in the main list
                      setAllPermissions((prev) => {
  ...
  // Ensure onSubmit and confirmDelete update `allPermissions` and then potentially trigger refetch or rely on useMemo
    ...
    const onSubmitEdit: SubmitHandler<PettyCashPermission> = async (data) => {
        ...
        try {
            ...
            const pdfFileInput = document.getElementById('pdf-file-edit') as HTMLInputElement; // Get input directly
            ...
            setAllPermissions(prev => prev.map(p => p.id === permissionToEdit.id ? response.data.data || {...p, ...data} : p)); // optimistic update or use response
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setAllPermissions(prev => prev.filter(p => p.id !== permissionToDelete));
    ...
    const handleViewPdf = (filename: string) => {
     ...
     const updateItemsTable = useCallback((link: { url?: string | null }, setLoadingCallback: (loading: boolean) => void) => {
        if (!link.url) return;
        setLoadingCallback(true); // Use the passed setLoading
        axiosClient(link.url) // Assumes link.url already includes necessary params like 'rows'
            .then(({ data }) => {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterMoment} adapterLocale={t("locale") || 'en'}>
      <Paper sx={{ mb: 2, p: 2 }}> {/* Filter Bar Paper */}
          ...
          {/* --- Approval Filters --- */}
           <Grid item xs={12} sm={6} md={3} lg={2}>
               <FormControl fullWidth size="small">
                   <InputLabel id="manager-approve-filter-label">اعتماد المدير</InputLabel>
                   <Select
           ...
           <Grid item xs={12} sm={6} md={3} lg={2}>
               <FormControl fullWidth size="small">
                   <InputLabel id="auditor-approve-filter-label">اعتماد المراجع</InputLabel>
                   <Select
        ...
        >
          <TableHead>
            <TableRow sx={{ '& th': { fontWeight: 'bold', backgroundColor: 'grey.200' } }}>
              ...
              <TableCell align="center">اعتماد المدير</TableCell> {/* Manager Approval */}
              <TableCell align="center">اعتماد المراجع</TableCell> {/* Auditor Approval */}
          ...
          <TableBody>
            ...
            {loading ? (
                <TableRow>
                 ...
                 <TableRow>
                    <TableCell colSpan={11} align="center" sx={{py: 4}}>
                       <Typography color="text.secondary">لا توجد أذونات صرف تطابق البحث.</Typography>
                    </TableCell>
                 </TableRow>
            ) : (
              ...
              <TableRow key={permission.id} hover>
                ...
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{dayjs(permission.date).format("DD-MM-YYYY")}</TableCell>
                <TableCell align="right">{formatNumber(permission.amount)}</TableCell>
                ...
                <TableCell>{permission.creditAccountNames}</TableCell>
                 ...
                 <TableCell align="center">
                  {permission.user_approved_time ? (
                    <Tooltip title={`Approved on: ${dayjs(permission.user_approved_time).format('YYYY-MM-DD HH:mm')}`}>
                    ...
                    <Button
                       ...
                       size="small"
                       color="primary"
                       onClick={() => handleApprove(permission.id, 'user_approved_time')}
                 ...
                 <TableCell align="center">
                  {permission.auditor_approved_time ? (
                     <Tooltip title={`Approved on: ${dayjs(permission.auditor_approved_time).format('YYYY-MM-DD HH:mm')}`}>
                       <Chip label="معتمد" color="secondary" size="small" variant="outlined" />
                    </Tooltip>
                  ) : (
                    <Button
                       variant="outlined"
                  ...
                  <Stack direction="row" spacing={0.5} justifyContent="center">
                    {permission.pdf_file && (
                      <Tooltip title={t("view_pdf")}>
                        <IconButton size="small" onClick={() => handleViewPdf(permission.pdf_file)}>
                          <GridVisibilityOffIcon fontSize="inherit"/>
                        </IconButton>
                      </Tooltip>
                    )}
                    <Tooltip title={"طباعة"}>
                      <IconButton size="small" href={`${webUrl}pettycash2/${permission.id}`} target="_blank">
                        <Printer size={18}/>
                      </IconButton>
                    </Tooltip>
                  </Stack>
                  ...
                  <Stack direction="row" spacing={0.5} justifyContent="center">
                     <Tooltip title={t("edit")}>
                       <span> {/* Span needed for tooltip on disabled button */}
                         <IconButton size="small" onClick={() => handleEdit(permission.id)} disabled={!!permission.user_approved_time || !!permission.auditor_approved_time}>
              ...
              return (
                <Grid item key={link.label || i}>
                  <MyLoadingButton
                    ...
                    onClick={(setLoadingCallback) => { // Expects setLoading passed from MyLoadingButton
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmitEdit)}> {/* Use specific submit handler */}
           <DialogContent>
                ...
                <Stack spacing={2} sx={{ mt: 1 }}>
                  ...
                  />
                    ...
                    <Button component="label" variant="outlined" size="small">
                       {t("pdf_file")} (Optional)
                        <input id="pdf-file-edit" type="file" hidden accept=".pdf,.jpg,.png,.jpeg" />
                         {/* Removed register - handle file via direct DOM access or state */}
                    </Button>
                     {/* Display current file if exists */}
                    {permissionToEdit.pdf_file && <Typography variant="caption">Current: {permissionToEdit.pdf_file}</Typography>}
