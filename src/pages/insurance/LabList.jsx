...
function LabList() {
  ...
  useEffect(()=>{
    if (activeCompany) {
      setTests(()=>{
        return [...activeCompany.tests.filter((test)=>test.main_test_name.toLowerCase().includes(search.toLowerCase()))]
  ...
  return (
    ...
    {activeCompany &&     <TableContainer >
      ...
      <a href={`${webUrl}company/test/${activeCompany.id}`}>التقرير</a>
          ...
          {activeCompany &&<Pagination  shape="rounded"  onChange={(e,number)=>setPage((number  * 10)- 10)} count={activeCompany ? (activeCompany.tests.length /10 ).toFixed(0) :  10}  variant="outlined" />}
        </TableContainer>}
      </Grid> 
      <Grid item xs={3}>
        <List>
