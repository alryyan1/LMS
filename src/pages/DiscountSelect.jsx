...
const DiscountSelect = ({id,disc,activePatient,setTests})=>{
  ...
  const changeDiscountHandler = async (id,dis)=>{
     ...
     const response = await fetch(`${url}labRequest/${activePatient.id}`, {
       headers: { "content-type": "application/json" },method:"PATCH",body:JSON.stringify( {test_id:id,discount:dis})
    ...
    return    <Select disabled={activePatient.is_lab_paid == 1} onChange={(e)=>{
