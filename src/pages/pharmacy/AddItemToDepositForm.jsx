...
function AddItemToDepositForm({
  ...
  const { selectedInvoice: selectedDeposit } = useOutletContext();
  const {
    setValue,
    register,
    reset,
...
const filterOptions = createFilterOptions({
  ...
  const submitHandler = async (formData) => {
    ...
    const payload = {
      ...
      expire: dayJsObj
        ? `${dayJsObj.year()}/${dayJsObj.month() + 1}/${dayJsObj.date()}`
        : null,
    ...
    axiosClient
      ...
      .then((data) => {
        console.log(data,'data is deposit item');
        setLoading(false);
        if (data.status) {
